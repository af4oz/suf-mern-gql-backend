...
export default {
  ...
  async getVotes(quesId: ObjectId) {
    const question = await QuestionModel.findById(quesId).populate('upvoteCount').populate('downvoteCount'); // TODO: avoid multiple populate
    return { upvoteCount: question?.upvoteCount, downvoteCount: question?.downvoteCount }
  },
