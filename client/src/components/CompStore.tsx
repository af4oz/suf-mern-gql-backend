...
import { MdClear as ClearIcon, MdErrorOutline as ErrorIcon } from 'react-icons/md';
...
const VButton = styled(Button)(({ variant }: { variant: "contained" | "outlined" }) => [
...
const Checkbox = ({ checkedIcon, checked, icon, onClick, ...otherProps }: CheckboxProps) => {
  return (
    <button
...
`
type IconButtonProps<T extends "button" | "a"> = { tag: T; styles?: TStyles; } & ComponentProps<T>;

declare function IconButtonFn<Tag extends "a" | "button">(props: IconButtonProps<Tag>): JSX.Element;

const IconButton = React.forwardRef<HTMLElement, IconButtonProps<any>>(function IconButton(props, ref) {

  const { tag, styles, href, children, ...rest } = props;
  let iconButton;
  if (tag === "a") {
    iconButton = (
      <RouterLink to={href} css={[iconButtonStyles, styles && styles.iconButton]} {...rest} ref={ref as RefObject<HTMLAnchorElement>}
      >
        {children}
      </RouterLink>
    )
...
const InputAdornment = React.forwardRef<HTMLDivElement, ComponentProps<'div'>>(function InputAdornment(props, ref) {
  return (
...
const TextField = React.forwardRef<HTMLElement, TextFieldProps<any>>(function TextField(props, ref) {
  const { InputProps = {}, fullWidth, styles, label, tag, error, helperText, ...rest } = props;
  ...
  return (
    <div css={[fieldRootStyles, fullWidth && tw`w-full`, styles && styles.fieldRoot]}>
      <div css={[inputRootStyles, fullWidth && tw`w-full`, styles && styles.inputRoot]} >
        ...
        {
          tag === "textarea" ? (
            <textarea ref={ref as RefObject<HTMLTextAreaElement>} {...rest} css={[inputStyles, styles && styles.input]} />
          ) : (
            <input type="text" ref={ref as RefObject<HTMLInputElement>} {...rest} css={[inputStyles, styles && styles.input]} />
          )
        }
...
type MenuItemProps<T extends "div" | "a"> = { tag: T; selected?: boolean } & ComponentProps<T>;

declare function MenuItemFn<T extends "div" | "a">(props: MenuItemProps<T>): JSX.Element;
...
const MenuItem = React.forwardRef<HTMLElement, MenuItemProps<any>>(function MenuItem(props, ref) {
...
const Menu = ({ open, anchorEl, anchorOrigin, transformOrigin, onClose, children, ...rest }: MenuProps) => {
...
interface AutoCompleteProps {
  ...
  onInputChange?: (event: SyntheticEvent, newValue: any, reason?: string, details?: any) => void;
  onChange?: (event: SyntheticEvent, newValue: any, reason?: string, details?: any) => void;
  renderTags?: (value: any, getTagProps: (params: { index: number }) => Partial<ChipWithCloseProps> & HTMLProps<HTMLElement>) => ReactNode;
  fullWidth?: boolean;
  ChipProps?: any;
}
const Autocomplete = React.forwardRef<HTMLInputElement, AutoCompleteProps>(function Autocomplete({ disabled, value: valueProp, inputValue: inputValueProp, onInputChange, onChange, renderInput, renderTags, fullWidth, ChipProps, ...rest }, ref) {
  ...
  const handleValue = (event: SyntheticEvent, newValue: any, reason: string, details?: any) => {
  ...
  const selectNewValue = (event: SyntheticEvent, option: any, reason = 'selectOption') => {
...
const Alert = React.forwardRef<HTMLDivElement, AlertProps>(function Alert(props, ref) {
  ...
  return (
    <div css={[tw`text-xs sm:text-sm flex items-center justify-between min-width[240px] p-1 md:p-2 text-white bg-gray-900 bg-opacity-75 shadow-sm rounded-sm  font-bold leading-5 tracking-wide `, bgStyles && bgStyles, styles && styles.alertRoot]}>
      ...
      {
        onClose ? (
          <IconButton tag="button" css={[tw`font-size[1.2em] color[inherit]`, styles && styles.alertEndIcon]} onClick={onClose}>
            <ClearIcon />
          </IconButton>
        ) : null
      }
...
const Tag = React.forwardRef<HTMLElement, TagProps<any>>(function Tag(props, ref) {
  const { href, label, styles, children, ...rest } = props;
  let tag;
  if (href) {
    tag = (
      <Link to={href} ref={ref} css={[tw`no-underline`, styles && styles.link]} {...rest}>
